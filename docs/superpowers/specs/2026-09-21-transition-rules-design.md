...
| **G4 생각 예산** | D4가 5에 도달 | 차단 없음. 사용자 메시지 한 번: `[gate] 8천 자 넘는 생각이 5번째다. 푼 실행은 4번을 넘긴 적이 없다. 머릿속 추적을 멈추고 스크립트로 옮겨라.` 이후 남은 스텝의 `reasoning_effort`를 `low`로 고정. **2026-09-22 수정: effort 하향은 뺐다**(아래 참고). 경고·`gate_warn` 이벤트·원장 줄·`long_reasoning_steps` 신호는 유지 | 실행 끝까지 유지 |
...
- G4의 effort 하향은 이 스펙에서 유일하게 모델 출력의 질에 손대는 개입이다. 리플레이로는 효과를 잴 수 없으므로(과거 실행은 medium이었다) 라이브 검증에서만 확인한다(§6). 효과가 없거나 나쁘면 경고만 남기고 하향은 뺀다.
  - **2026-09-22 결과: 하향 제거.** 첫 라이브 발동 두 건(각 120분)에서 경고 + `low` 이후 스텝당 생각 길이가 줄지 않고 오히려 늘었다. ROVM: 중앙값 2,201 → 15,631자(이후 29스텝 중 20개가 8천 자 초과, 122분에 54스텝뿐). damnida: 중앙값 720 → 5,382자(이후 67스텝 중 26개가 8천 자 초과). 쓸모없거나 해로울 가능성이 있어 `effort_override`를 코드에서 제거했다. 경고 메시지, `gate_warn` 이벤트, 원장 줄, `long_reasoning_steps` 신호는 그대로다.
