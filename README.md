...
| quiz/captain-hook (Dreamhack, Windows PE) | real | unsolved after 4 static runs (host ×2, sandbox ×2, ~500 steps); sandbox run 8 (89 steps, 59.6 min, 2 compactions) with the `actions`-capable `run_gui` produced a full 16-glyph reading (`DH{4K58900003000000}`, via a WM_PAINT-posting helper + proxy gdiplus.dll log) that Dreamhack **rejected**; the run advanced the display by forcing paints instead of real clicks, so the many repeated 0 glyphs are suspect (investigation in progress). Runs 5–7 were stopped to fix run_gui defects they exposed: no right-click, silent xdotool failures, stale wine windows, unreported blank captures |
